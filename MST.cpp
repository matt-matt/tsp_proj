...
void MST::printMST() {
    ...
    float avg = calMean(MST_1);
    cout<<"Mean edge weight of MST"<<endl;
    cout<<avg<<endl;
    cout<<"Standard Deviation of edge weight in MST"<<endl;
    cout<<calStd(MST_1, avg)<<endl;
}
...
float MST::calMean(int option) {
	...
	if(option == MST_1) {
        ...
        for (int i = 1; i < N; i++) {
            mean += adjacentMatrix[i][parent[i]];
        }
...
float MST::calStd(int option, int mean) {
	float std = 0.0;
    float variance = 0.0;
	...
	if(option == MST_1) {
        ...
        for (int i = 1; i < N; i++) {
            variance += (float)pow(adjacentMatrix[i][parent[i]] - mean, 2);
        }
        variance /= N;
        std = sqrt(variance);
	} else if(option == TSP2) {
